...
const ProductCard = ({ product }: { product: Product }) => {
    ...
    const img = product.images?.[0]?.src || product.image_url;
    const source = product.source || "noon";
    ...
    return (
        ...
        >
            <div className="relative bg-card border border-border rounded-lg p-4 flex flex-col h-full shadow-sm hover:shadow-lg transition-shadow duration-200">
                <button
                    className="absolute top-3 right-3 z-10 p-1.5 border border-border rounded-full bg-white group-hover:border-primary"
                    onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                    }}
            ...
            <div className={`container ${landingPage ? "mx-auto px-4" : ""}`}>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {products.map((product, idx) => (
                        <ProductCard key={product._id ?? product.product_url ?? idx} product={product} />
                    ))}
                </div>
            </div>
        </section>

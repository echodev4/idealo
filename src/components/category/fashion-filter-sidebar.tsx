...
export type FashionFacets = Record<FashionFacetKey, string[]>;

export interface FashionFilterSidebarProps<
...
> {
    ...
    onToggle: (facetKey: FashionFacetKey, value: string) => void;
    onReset: () => void;
    ...
    products: T[];
    filteredProducts: T[];
    getFacetValue: (p: T, facetKey: FashionFacetKey) => string | null;
}
...
>(props: FashionFilterSidebarProps<T>) {
    ...
    const countForOption = (facetKey: FashionFacetKey, value: string) => {
        ...
        return products.filter((p) => {
            ...
            // other facets apply normally; for this facetKey, treat as (selected OR this value)
            for (const key of Object.keys(selected)) {
                const k = key as FashionFacetKey;
            ...
            // if this facet has no selections, count as if selecting this value
                ...
                <div className="p-4">
                    ...
                    <Section title="Fit">
                        <FacetList
                            facetKey="Fit"
                            values={facets["Fit"]}
                            selected={selected["Fit"] ?? new Set()}
                            onToggle={(v) => onToggle("Fit", v)}
                            countForValue={(v) => countForOption("Fit", v)}
                        />
                    </Section>
                    ...
                    <Section title="Occasion">
                        <FacetList
                            ...
                            selected={selected["Occasion"] ?? new Set()}
                            onToggle={(v) => onToggle("Occasion", v)}
                            countForValue={(v) => countForOption("Occasion", v)}
                        />
                    </Section>
                    ...
                    <Section title="Pattern">
                        <FacetList
                            ...
                            selected={selected["Pattern"] ?? new Set()}
                            onToggle={(v) => onToggle("Pattern", v)}
                            countForValue={(v) => countForOption("Pattern", v)}
                        />
                    </Section>
